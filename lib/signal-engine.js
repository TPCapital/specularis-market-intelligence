...
export function buildSignalEngine({ indices = [], scanner = [], breadth = null, risk = null, earnings = [], insider = [], relativeVolume = [] } = {}) {
  ...
  const earningsRisk = (earnings || []).length >= 6 ? 6 : (earnings || []).length >= 3 ? 3 : 0;
  const insiderBias = (insider || []).filter((item) => item.sentiment === "BULLISH").length - (insider || []).filter((item) => item.sentiment === "BEARISH").length;
  const rvolExpansion = (relativeVolume || []).filter((item) => item.signal === "RVOL_EXPANSION").length;

  let riskAppetite = clamp(50 + qqq * 7 - Math.max(0, vix) * 4 - Math.max(0, tnx) * 2 - Math.max(0, dxy) * 2 + (breadthScore - 50) * 0.4 + insiderBias * 1.5 + rvolExpansion * 1.2 - earningsRisk);
