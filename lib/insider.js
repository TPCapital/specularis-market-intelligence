...
export async function buildInsiderLayer({ symbols = "", finnhubKey = "" } = {}) {
  ...
  const rows = await Promise.all(list.map(async (symbol) => {
    try {
      const payload = await fetchJson(`https://finnhub.io/api/v1/stock/insider-transactions?symbol=${encodeURIComponent(symbol)}&token=${encodeURIComponent(finnhubKey)}`, { timeoutMs: 9000 });
