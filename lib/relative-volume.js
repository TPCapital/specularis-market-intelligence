...
function classify(relativeVolume, hasRealVolume, changePercent) {
  if (!hasRealVolume && relativeVolume < 1.2) return "PROXY_ONLY";
...
export function buildRelativeVolumeLayer({ quotes = [], tradingView = [], premarketMomentum = [] } = {}) {
  ...
  const leaders = (quotes || [])
    ...
    .map((item) => {
      ...
      const score = clamp(Math.round(Math.min(relativeVolume, 3) * 25 + Math.min(Math.abs(changePercent), 6) * 5 + (hasRealVolume ? 10 : 0)));
      return {
        symbol: item.symbol,
