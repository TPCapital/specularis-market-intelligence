...
export function buildTradeDecision({
  ...
  const summary = top
    ...
    : '暂无高质量交易目标，等待开盘量价确认。';
  const marketGateBlocksTrading = grade === 'NO TRADE' || permission === '禁止交易';
  const finalTargets = marketGateBlocksTrading
    ? targets.map((item) => ({
        ...item,
        grade: item.direction === 'PUT / HEDGE' ? 'HEDGE WATCH' : 'WATCH',
        confidence: '低',
        optionStyle: item.direction === 'PUT / HEDGE' ? '对冲观察，不主动开仓' : '观察，不主动开仓',
        risk: '高：市场闸门未通过，只观察',
        reasons: [...(item.reasons || []), 'Market gate 禁止交易：降级为观察信号']
      }))
    : targets;
  const finalActionableTargets = marketGateBlocksTrading ? [] : actionableTargets;
  const finalTop = finalActionableTargets[0] || finalTargets[0] || null;
  const focus = finalTargets.slice(0, 5).map((item) => `${item.symbol}｜${item.grade}｜${item.direction}｜${item.entryTrigger}`);
  const avoid = [
  ...
  const checklist = [
    ...
    { label: '有可交易目标', pass: finalActionableTargets.length > 0, detail: marketGateBlocksTrading ? 'Market gate 禁止交易，全部降级观察' : `${finalActionableTargets.length} 个 B级以上` },
  ...
  return {
    ...
    probability: clamp(Math.round(baseScore * 0.78 + finalActionableTargets.length * 4)),
    direction,
    permission,
    actionBias,
    title,
