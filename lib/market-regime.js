...
export function buildMarketRegime({ marketData = {}, premarketMomentum = {}, marketBreadth = {}, tradingView = [] } = {}) {
  ...
  let score = 50 + spy * 5 + qqq * 8 - Math.max(0, vix) * 4 - Math.max(0, dxy) * 2 - Math.max(0, tnx) * 2 + (breadthScore - 50) * 0.18 + leaderCount * 2 + tvCount * 0.8;
  ...
  return {
    ...
    explanation: `SPY ${spy.toFixed(2)}%，QQQ ${qqq.toFixed(2)}%，VIX ${vix.toFixed(2)}%，市场宽度 ${Math.round(breadthScore)}，动能股 ${leaderCount} 个。`
  };
}
