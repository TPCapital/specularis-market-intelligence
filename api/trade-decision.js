 ...
 * /api/trade-decision
 * ────────────────────
 * Returns the current trade decision grade + execution permission.
 * Lightweight endpoint: reuses snapshot data, extracts tradeDecision layer.
 */
...
export default async function handler(req, res) {
  try {
    ...
    const tradeDecision =
      ...
      snapshot.sources?.tradeDecision?.data ||
      snapshot.sources?.decisionEngine?.data?.tradeDecision ||
      null;
