...
async function loadQuotes(symbols, token) {
  ...
  return Promise.all(originalSymbols.map(async (symbol, index) => {
    ...
    try {
      ...
      const pricePayload = await fetchJson(`${TWELVEDATA_BASE}/price?symbol=${encodeURIComponent(provider)}&apikey=${encodeURIComponent(token)}`, { timeoutMs: 4200 });
      ...
      let quotePayload = {};
      try {
        quotePayload = await fetchJson(`${TWELVEDATA_BASE}/quote?symbol=${encodeURIComponent(provider)}&apikey=${encodeURIComponent(token)}`, { timeoutMs: 4200 });
      } catch (error) {
